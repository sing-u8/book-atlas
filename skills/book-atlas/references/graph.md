...
5. **브라우저가 막고 있을 수 있다.** `obsidian://` 은 외부 앱 프로토콜이라 브라우저가 "이 사이트가
   ...
   먼저 실행한 뒤 다시 클릭한다.
6. **정보 패널만 열리는데 vault 이름도 맞다 = 아직 시작하지 않은 챕터일 수 있다.**
   `_chapter.md` 가 아직 없는 챕터 노드는 애초에 `path` 없이 빌드된다 — 이건
   고장이 아니라 정상 폴백이다(진행률 `done/total` 만 보여준다). 그 챕터 정리를
   시작해 `_chapter.md` 가 생기고 재빌드하면 그때부터 클릭이 Obsidian 을 연다.
   반대로 **Obsidian 이 "새 파일을 만들까요"를 묻는다면** = 그 노드는 `path` 를
   갖고 있는데 가리키는 파일이 실제로 없다는 뜻이다 — 노트를 옮기거나 이름을
   바꾼 뒤 재빌드를 안 한 상태다. 만들지 말고 빌드를 다시 돌린다.
...
| 얇은 언급 엣지 | 본문의 `[[슬러그]]` 링크 |
| 개념 노드·정의 | `_glossary.md` 항목과 그 첫 줄(120자 절단) |
