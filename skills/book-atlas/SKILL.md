...
1. **CWD 자체가 vault 루트인지 먼저 본다** — `_global/config.md` 가 CWD 에 바로
   있으면(즉 CWD 가 이미 `{book-slug}-atlas/` 안이면) 그 저장소를 후보로 삼는다.
   아래 글롭은 CWD 의 **하위** 디렉터리만 찾으므로 CWD 자신이 vault 루트인
   경우는 매치하지 않는다 — 이 단계가 없으면 그 저장소를 영영 못 찾는다.
2. 위에서 못 찾았으면 `**/*-atlas/_global/config.md` 글롭으로 기존 저장소를 찾는다
   (`node_modules/`·`.git/` 제외).
   - **1개** → 사용자에게 확인받고 활성 저장소로 삼는다.
   - **여러 개** → 후보를 보여주고 고르게 한다.
3. **저장소 없음 + CWD 에 PDF 있음** → 아래 **온보딩**. 사용자가 PDF 경로를 인자로
...
5. **챕터 완결 시** — `_chapter.md` 를 완성한다(필수 요소 5개: `templates.md` ④).
   **챕터 개요의 `## 읽는 순서` 흐름도는 노드 수와 무관하게 항상 mermaid flowchart
   ...
   출력에 `행 경계 밖 N건 제외` 가 함께 나오면, 커버리지의 `NN행` 표기를 기준으로 앞 절의
   꼬리와 다음 절의 머리를 뺐다는 뜻이다(SKILL-010). **그 수가 유난히 크면 행 표기를
   의심한다** — 행 경계가 틀리면 진짜 후보가 조용히 빠지므로 일부러 항상 인쇄한다.
