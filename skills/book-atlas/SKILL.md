...
description: >
  ...
  각 노트의 `## 스스로 확인` 질문에 대한 모범답안 요청도 이 스킬이 받는다. 다만 반복
  퀴즈 루프·꼬리질문 사슬·약점 추적은 하지 않는다 — gap 파일도 세션 로그도 없다.
---
...
3. **인출 훈련 체계는 두지 않는다.** `## 스스로 확인` 질문을 심고, 요청하면 그
   모범답안을 노트와 원문에서 끌어와 쓴다. 그러나 반복 퀴즈 루프·꼬리질문 사슬·
   막힘 추적은 하지 않는다 — gap 파일도 세션 로그도 없다.
...
사용자가 그 질문에 혼자 답해 본 뒤 **모범답안을 요청**하는 것이 이 스킬의 가장 흔한
마무리다. 이렇게 만든다.

- **노트와 원문에서만 끌어온다.** 노트가 담은 것으로 답이 안 되면 그 절의 원문을
  manifest 의 오프셋으로 다시 뽑아(`pdftotext -layout`) 확인한다. 거기에도 없으면
  **책이 답하지 않는 질문이라고 밝힌다** — 지어내지 않는다. `## 스스로 확인` 은 원래
  책 너머를 묻는 질문도 담으므로 이 경우가 드물지 않다.
- 근거를 **`(책 p.NNN)`** 로 단다. 노트의 어느 대목에서 온 것인지도 함께 가리킨다.
- 질문의 형태에 맞춘다 — "왜"를 물으면 이유를, "어떻게 재는가"를 물으면 절차를,
  "어느 쪽인가"를 물으면 판단 기준과 그 근거를 답한다.
- 사용자가 자기 답을 함께 보여주면 **무엇이 빠졌는지만** 짚는다. 점수를 매기지 않고
  잘한 점을 늘어놓지 않는다.

**모범답안은 대화로만 낸다 — 노트 파일에 쓰지 않는다.** 노트 위쪽은 6개 H2 로 고정된
스키마라 답안이 들어갈 자리가 없고, 구분자 아래는 사용자 영역이다(하드 룰 4).
남기고 싶다는 요청을 받으면 `## 내 메모` 에 직접 옮기라고 안내한다.

반복 퀴즈 루프·꼬리질문 사슬·약점 추적은 이 스킬의 몫이 아니다 — **막힘을 추적하거나
기록하지 않는다.** gap 파일도 세션 로그도 없다.
