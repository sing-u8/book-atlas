...
# 대상: vault 안의 절 노트 — 아래 is_target 의 네 조건을 전부 만족하는 파일
...
# has_glossary_ancestor <디렉터리> — 자기 자신부터 위로 4단계까지 훑어
# `_glossary.md` 를 가진 디렉터리가 있으면 0. 그 디렉터리가 vault 루트다.
...
# 있고, 여유 두 단계를 더 본다.
has_glossary_ancestor() {
  dir="$1"
  i=0
  while [ "$i" -lt 4 ]; do
    [ -f "$dir/_glossary.md" ] && return 0
    parent="${dir%/*}"
    [ -z "$parent" ] && parent="/"
    [ "$parent" = "$dir" ] && return 1
    dir="$parent"
...
has_glossary_ancestor "$dir" || exit 0
